...
3. `newsletter_drain.yml` corre `--resume` cada hora entre las **21:15 y las 04:15 UY** de los días de envío (martes y viernes de noche). El primer intento cae 15 minutos después del corte, así los pendientes reciben el mail esa misma noche. Hay además un **barrido diario a las 09:15 UY**: si el envío se disparó muy tarde, la cola puede nacer después de la ventana nocturna y así se levanta igual.
...
- **La cola vence a las 20 horas** (`QUEUE_MAX_AGE_HOURS`): no tiene sentido mandar "los partidos de este finde" cuando el finde ya pasó. Si vence con gente adentro, el workflow **falla a propósito** y llega el aviso de GitHub. El barrido diario garantiza que ese aviso llegue dentro de las 24h incluso si el envío salió con horas de retraso (el del 11/09 salió 3h17 tarde).
- **El mail de control del jueves reporta la cola:** dice si el último envío llegó a todos, si hay pendientes esperando las 21:15, o si quedaron perdidos por cupo insuficiente.
- **El cupo diario se reserva para el newsletter:** las bienvenidas no corren martes ni viernes y el monitor pasó al jueves. Los días del cron son UTC, así que la pausa tapa exactamente el día de cupo del envío y las bienvenidas se reanudan a las 00:40 UTC = **21:40 de Uruguay**: a quien se anota un martes o un viernes le llega esa misma noche, no al día siguiente.
- **Orden en la noche del envío:** el drenado corre 21:15 y las bienvenidas 21:40, así el cupo recién renovado va primero a los suscriptores que no recibieron el newsletter.
...
**Fase Título / Permanencia (divisionales de 16 — Mayores, Reserva, Sub-20, Presenior, Más 40):** al terminar el todos contra todos (15 fechas) la divisional se parte en dos grupos de 8 que juegan 7 fechas más, arrastrando los puntos. En T112 la liga usó `T2/AT` para "DIVISIONAL A TITULO"; en T113 CLT quedó arriba en Reserva, Presenior y Más 40, y abajo en Mayores y Sub-20. Al 22/09/2026 la liga todavía no había publicado esas series.

- **Próximos antes de la fecha 1:** la tabla de posiciones de una serie nueva viene vacía hasta que se juega un partido, así que antes el extractor no la detectaba y los próximos no aparecían. Ahora, si la tabla está vacía (o con PJ ≤ 1) y CLT figura en `partidos/api.php` de esa serie, la guarda en la tabla SQLite `league_clt_series` y el generador la consulta igual. No se hace en el brute-force de respaldo para no alargarlo.
- **Nombre de la fase cuando la serie nueva todavía no está en el Sistema A (`_split_stage` en `json_generator.py`):** se deduce de dónde terminó CLT la fase regular — mitad de arriba "Fase Título", mitad de abajo "Permanencia y Descenso". Se usa para el encabezado y el separador de "Partidos" y para el `stage` de la tabla nueva en `league_context.json` (sin esto la tabla nueva quedaba rotulada como fase regular). Cuando el Sistema A carga la serie con su nombre real, manda ese.
- **Numeración:** si ningún partido de una fase trae número de fecha (fase recién publicada, solo en el Sistema B), se numeran 1, 2, 3… dentro de la fase en vez de 16, 17… Si alguno ya trae número no se adivina nada (con fechas postergadas podría quedar mal).
- **Tab Tablas (`currentPhaseTables` en `app/actualidad/page.tsx`):** si una categoría ya tiene tabla de una fase posterior con menos equipos, se oculta la de la fase regular (el otro grupo no interesa). Si todavía no la tiene pero la fase regular de 14+ equipos terminó, se muestra solo la mitad donde quedó CLT, renumerada 1–8 (sin texto explicativo: Tomás prefirió que no aparezca).
